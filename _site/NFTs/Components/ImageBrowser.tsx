...
import {DateFormat, NFT} from "../../types.d.ts";
import {gfm, h} from "../../deps.ts";
import {ContenPageProps} from "./DetailPage.tsx";

export default function ImageBrowser({nft, config}: ContenPageProps) {
    return (
        <div className="max-w-screen-sm px-6 pt-8 mx-auto">
            <div className="pb-5">
                <a
                    href="/"
                    className="inline-flex items-center gap-1 text-sm text-gray-500/80 hover:text-gray-700 transition-colors"
            ...
            <div>
                <h1 className={'text-center text-3xl font-semibold'}>{nft.title}</h1>
                <image className='mt-12' src={nft.nftImage}></image>
            </div>
